...
.PHONE: print-version
print-version:
	@echo "Version: $(git_version_tag)"
	@echo "Tag: $(git_tag)"
	@echo "SHA: $(git_sha)"

.PHONY: build
build: $(BINDIR)/$(TARGET) ## Build the main target

.PHONY: release
release: $(foreach os, linux, $(foreach arch, amd64 arm64, $(OUTPUTDIR)/$(TARGET)_$(os)_$(arch)))
...
$(BINDIR)/$(TARGET): $(go_src) $(generated_files)
	$(go_env) go build $(go_ldflags) -o $(BINDIR)/$(TARGET) .


$(foreach os, linux, $(foreach arch, amd64 arm64, $(OUTPUTDIR)/$(TARGET)_$(os)_$(arch))): $(go_src) $(generated_files)
	$(go_env) go build $(go_ldflags) -o $@ .
